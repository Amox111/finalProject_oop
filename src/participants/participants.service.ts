...
export class ParticipantsService {
  ...
  findByEventId(eventId: string): Participant[] {
    this.eventsService.findById(eventId); // ตรวจว่า event มีอยู่จริงก่อน
  ...
  create(dto: CreateParticipantDto): Participant {
    // ตรวจว่า event มีจริง + ยังมีที่ว่าง (ถ้าเต็ม throw 400)
    this.eventsService.incrementParticipants(dto.eventId);

    // ตรวจ email ซ้ำในงานเดียวกัน (ยกเว้น CANCELLED ลงใหม่ได้)
    ...
    if (isDuplicate) {
      this.eventsService.decrementParticipants(dto.eventId); // rollback
    ...
    const newParticipant: Participant = {
      ...
      role: dto.role,
      status: ParticipantStatus.REGISTERED, // system กำหนด ไม่ให้ user set
  ...
  update(id: string, dto: UpdateParticipantDto): Participant {
    ...
    // auto-set timestamp เมื่อ status เปลี่ยน
    const confirmedAt =
  ...
  remove(id: string): void {
    ...
    this.eventsService.decrementParticipants(participant.eventId); // ลด capacity ด้วย
